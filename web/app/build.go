...
import (
	...
	"qlova.org/seed/new/html/link"
	"qlova.org/seed/new/html/meta"
	"qlova.org/seed/new/html/style"
	"qlova.org/seed/new/html/title"
	"qlova.org/seed/new/repeater"

	script_html "qlova.org/seed/new/html/script"
)
...
func (a App) build() {
	...
	app.worker.Assets = asset.Of(a.Seed)
	a.Seed.Save(app)
		...
		script_html.New(
			...
			document.addEventListener('DOMContentLoaded', async function() { 
				...
				function launch(url, title, w, h) {
					...
					launched = window.open(url, title, `+"`"+`toolbar=no, location=no, directories=no, status=no, menubar=no, scrollbars=no, resizable=no, copyhistory=no, width=${w}, height=${h}, top=${y}, left=${x}`+"`"+`);
				}
